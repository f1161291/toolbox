...
root_user(){
  ...
  [ -z "$password" ] && { red "密码未设置，将使用随机生成的root密码"; password=$(cat /proc/sys/kernel/random/uuid); }
...
install_acme(){
    ...
    if [[ -z $acmeEmail ]]; then
        ...
        acmeEmail="${autoEmail}@gmail.com"  # 修复自动生成邮箱逻辑
...
acme_standalone(){
    ...
    check_80
    WARPv4Status=$(curl -s4m10 https://www.cloudflare.com/cdn-cgi/trace -k 2>/dev/null | grep warp | cut -d= -f2)
    WARPv6Status=$(curl -s6m10 https://www.cloudflare.com/cdn-cgi/trace -k 2>/dev/null | grep warp | cut -d= -f2)
    if [[ $WARPv4Status =~ on|plus ]] || [[ $WARPv6Status =~ on|plus ]]; then
        wg-quick down wgcf >/dev/null 2>&1
    fi
    
    ipv4=$(curl ifconfig.me 2>/dev/null)
    ipv6=$(curl 6.ipw.cn 2>/dev/null)
    ...
    domainIP=$(curl -sm8 ipget.net/?ip="${domain}" 2>/dev/null)
    
    if [[ $domainIP == $ipv6 ]]; then
        bash ~/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --install-cert -d "${domain}" --key-file /root/key.pem --fullchain-file /root/cert.pem --ecc
    fi
...
acme_cfapiTLD(){
    ...
    if [[ -z $ipv4 ]]; then
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "${domain}" -k ec-256 --install-cert -d "${domain}" --key-file /root/key.pem --fullchain-file /root/cert.pem --ecc
    else
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "${domain}" -k ec-256 --install-cert -d "${domain}" --key-file /root/key.pem --fullchain-file /root/cert.pem --ecc
    fi
    checktls
...
acme_cfapiNTLD(){
    ...
    export CF_Email="$CFemail"
    if [[ -z $ipv4 ]]; then
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "*.${domain}" -d "${domain}" -k ec-256 --install-cert -d "${domain}" --key-file /root/key.pem --fullchain-file /root/cert.pem --ecc
    else
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "*.${domain}" -d "${domain}" -k ec-256 --install-cert -d "${domain}" --key-file /root/key.pem --fullchain-file /root/cert.pem --ecc
    fi
    checktls
...
checktls() {
    if [[ -f /root/cert.pem && -f /root/key.pem ]]; then
        if [[ -s /root/cert.pem && -s /root/key.pem ]]; then
            ...
            green "证书申请成功! 证书(/root/cert.pem)和私钥(/root/key.pem)已保存到/root目录"
            yellow "证书路径: /root/cert.pem"
            yellow "私钥路径: /root/key.pem"
            acme_back2menu
        else
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
                wg-quick up wgcf >/dev/null 2>&1
            ...
            red "证书申请失败"
            green "建议如下: "
            yellow "1. 80端口模式请关闭防火墙或放行80端口"
            yellow "2. 同一域名多次申请可能触发Let's Encrypt风控，可切换证书颁发机构后重试"
            yellow "3. 可更换域名或等待7天后再尝试"
            acme_back2menu
        fi
    fi
}
...
revoke_cert() {
    ...
    if [[ -n $(bash ~/.acme.sh/acme.sh --list | grep "$domain") ]]; then
        ...
        rm -f /root/cert.pem /root/key.pem 2>/dev/null
        green "撤销${domain}证书成功"
        acme_back2menu
    else
        red "未找到${domain}的证书, 请检查域名输入是否正确"
        acme_back2menu
    fi
}
...
renew_cert() {
    ...
    if [[ -n $(bash ~/.acme.sh/acme.sh --list | grep "$domain") ]]; then
        ...
        red "未找到${domain}的证书，请检查域名输入"
        acme_back2menu
    fi
}

switch_provider(){
    yellow "请选择证书提供商, 默认使用 Letsencrypt.org"
    yellow "申请失败可切换 BuyPass.com 或 ZeroSSL.com"
    ...
    case "$provider" in
        2) bash ~/.acme.sh/acme.sh --set-default-ca --server buypass && green "切换为 BuyPass.com 成功！" ;;
        3) bash ~/.acme.sh/acme.sh --set-default-ca --server zerossl && green "切换为 ZeroSSL.com 成功！" ;;
        *) bash ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt && green "切换为 Letsencrypt.org 成功！" ;;
    esac
    acme_back2menu
}

uninstall_acme() {
    [[ -z $(~/.acme.sh/acme.sh -v 2>/dev/null) ]] && yellow "未安装Acme.sh, 无需卸载!" && exit 1
...
acme_menu() {
    ...
    echo "#############################################################"
    echo ""
    echo -e " ${GREEN}1.${PLAIN} 安装 Acme.sh 证书申请脚本"
    echo -e " ${GREEN}2.${PLAIN} ${RED}卸载 Acme.sh 证书申请脚本${PLAIN}"
    echo " -------------"
    echo -e " ${GREEN}3.${PLAIN} 申请单域名证书 ${YELLOW}(80端口申请)${PLAIN}"
    echo -e " ${GREEN}4.${PLAIN} 申请单域名证书 ${YELLOW}(CF API申请)${PLAIN} ${GREEN}(无需解析)${PLAIN} ${RED}(不支持freenom)${PLAIN}"
    echo -e " ${GREEN}5.${PLAIN} 申请泛域名证书 ${YELLOW}(CF API申请)${PLAIN} ${GREEN}(无需解析)${PLAIN} ${RED}(不支持freenom)${PLAIN}"
    echo " -------------"
...
main_menu(){
    ...
    case "$NumberInput" in
        ...
        l) bash -c "$(curl -sSL https://www.linkease.com/rd/fastpve/)" ;;  # 修复全角空格
