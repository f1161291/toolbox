...
generate_password() {
    tr -dc 'A-Za-z0-9!@#$%^&*()_+' < /dev/urandom 2>/dev/null | head -c 20 2>/dev/null || \
...
run_script() {
    ...
    if ! wget -N --no-check-certificate -q "$url" -O "$name" || [[ ! -s "$name" ]]; then
        red "下载失败或文件为空: $url"
        rm -f "$name"
        return 1
    fi
...
install_frp() {
    ...
    if [[ "$frp_type" == "frps" ]]; then
        ...
        if wget -q --no-check-certificate \
            ...
            -O ./install-frps.sh && [[ -s ./install-frps.sh ]]; then
            chmod 700 ./install-frps.sh
            green "✅ frps 脚本下载成功，开始安装..."
            bash ./install-frps.sh install
        else
            red "❌ frps 脚本下载失败或文件为空，请检查网络连接"
            rm -f ./install-frps.sh
        fi
    else
...
frp_menu() {
    while true; do
        ...
        case "$frp_choice" in
            1) install_frp "frps"; return ;;
            2) install_frp "frpc"; return ;;
            0) return ;;
            *) red "无效选项！" && sleep 1 ;;
        esac
    done
...
menu() {
    while true; do
        ...
        case "$choice" in
            ...
            u) run_script "https://raw.githubusercontent.com/f1161291/toolbox/main/tool.sh" "tool.sh" ;;
