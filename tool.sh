...
# ==================== 系统检测 ====================
detect_os() {
    local os_info=$(grep -i pretty_name /etc/os-release 2>/dev/null | cut -d '"' -f2)
...
root_user() {
    local os=$(detect_os)
    [[ "$os" == "Unknown" ]] && red "不支持当前系统，请使用主流操作系统" && return 1
    ...
    [[ ! -f /etc/ssh/sshd_config ]] && sudo $pkg_update && sudo $pkg_install openssh-server
    [[ -z $(type -P curl) ]] && sudo $pkg_update && sudo $pkg_install curl
...
run_script() {
    ...
    wget -N --no-check-certificate -q "$url" -O "$name" || {
        red "下载失败: $url"
        return 1
    }
    chmod +x "$name"
